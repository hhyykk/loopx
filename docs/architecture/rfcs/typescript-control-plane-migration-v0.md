...
| Bridge code added | 497 product LOC are bounded compatibility code: compact Python authority projection plus one managed-runtime request, the compatibility import, and the shared Python/TypeScript lock protocol. The projection and import exit with the top-level Node CLI; the dual lock exits with the remaining lease writers and fences. |
...
| Product-code net change | Product code is +1,622/−977 LOC, net +645. Tests and fixtures are reported separately at +794/−1,080; build configuration is +4. |
