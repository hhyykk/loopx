...
| Bridge code added | 476 Python diff LOC used only by the bounded bridge: 438 lines across `_mapping`, `_monitor_candidate`, `_due_monitor_candidates`, `_vision_wait_state`, `_registry_due_monitor`, `_decision_packet`, `_observation_packet`, `_index_digest`, `_native_result`, `_request`, `build_quota_monitor_poll_event`, `find_quota_monitor_poll_turn`, `_status_with_monitor_poll`, `_reload_status_after_monitor_writeback`, `_monitor_poll_failure`, `_capability_declaration_retry`, and `record_quota_monitor_poll_for_decision`, plus 38 import/schema wiring lines. The 34-line `_provider_writeback` is excluded because it adapts the retained real provider. |
...
| Product-code net change | 2,694 added minus 831 deleted product LOC, net +1,863; tests/examples are separately +623 and docs are excluded. The temporary increase buys one complete transaction and cannot repeat: the next deletion is the 476-line bridge when quota decision, Todo persistence, status projection, and remaining index writers are native. |
...
| Correctness and performance | Identity/admission, effect isolation, provider fencing, malformed receipts, concurrent CAS, crash repair, packaging, and launcher coverage pass. Managed-runtime cold start is 312.50/523.48 ms p50/p95, warm event 1.27/1.78 ms, durable commit 2.25/5.67 ms, and memory is 126.5 MiB idle/after burst. In 64 interleaved full-CLI pairs, Todo write baseline/candidate is 725.67/918.24 versus 754.25/1,097.63 ms p50/p95; replay is 661.28/751.15 versus 671.45/761.76 ms. Replay and median write stay within the threshold, while write p95 is an explicit owner-review hold. |
